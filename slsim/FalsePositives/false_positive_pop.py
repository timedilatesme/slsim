...
class FalsePositivePopBase(object):
    ...
    This class provides common functionality for drawing deflectors and sources,
    and can be extended to create specific types of false positive populations.
    """

    def __init__(self, elliptical_galaxy_population, cosmo=None, los_pop=None, field_galaxy_population=None):
        self.cosmo = cosmo
    ...
    def draw_field_galaxies(self, area, z_max = None):
        ...
        :param z_max: Maximum redshift for the field galaxies. If None, no redshift cut is applied.
    ...
    def add_field_galaxies_to_sources(self, sources, area, z_max=None):
        ...
        :param area: Area in which to draw field galaxies (in square arcseconds).
        :param z_max: Maximum redshift for the field galaxies. If None, no redshift cut is applied.
        :return: Updated list of source instances including field galaxies.
        """
        if self._field_galaxy_population is None:
            return sources
    ...
    def draw_false_positive(self, number=1):
        ...
        for _ in range(number):
            ...
            while not successful:
                ...
                source = self.add_field_galaxies_to_sources(sources=source, area=test_area*20, z_max=z_max)
...
class FalsePositivePointSourcePop(FalsePositivePopBase):
    """Class to perform samples of false positive populations with multiple point source types.

    This class generates configurations consisting of a central deflector and 
    multiple point sources drawn from provided populations (e.g. stars, quasars).
    """
    ...
    def draw_sources(self, z_max, theta_e):
        """
        Draws sources from all populations based on the number choices and positions them.

        :param z_max: Maximum redshift for the sources.
        :param theta_e: Einstein radius of the deflector (used for scaling positions).
    ...
    def draw_false_positive(self, number=1):
        ...
        for _ in range(number):
            ...
            while not successful:
                ...
                # add field galaxies to sources if field galaxy population is provided
                sources = self.add_field_galaxies_to_sources(sources=sources, area=test_area*20, z_max=z_max)
                ...
                false_positive = FalsePositive(
                    ...
                    cosmo=self.cosmo,
                    include_deflector_light=self._include_deflector_light,
                    los_class=self.los_config.draw_los(source_redshift=z_max,
                                                       deflector_redshift=deflector.redshift)
                )
                false_positive_population.append(false_positive)
                successful = True
