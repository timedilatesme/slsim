...
galaxy_simulation_pipeline = pipelines.SkyPyPipeline(skypy_config=None, sky_area=sky_area)
...
loaded_qso_host_catalog = Table.read(os.path.join(path, "../TestData/qso_host_catalog.fits"))
...
def test_false_positive_core_and_area():
    """Tests single population, area clustering, field galaxies, and base methods."""
...
def test_false_positive_multiple_and_ring():
    """Tests list of populations, list choices, weights, multiple items, and ring clustering."""
    fp_pop = FalsePositivePop(
        central_galaxy_population=lens_galaxies,
        source_populations=[source_galaxies],
    ...
    for i in range(fp.source_number):
        ...
        assert 0.5 * fp.einstein_radius_infinity <= r <= 2.5 * fp.einstein_radius_infinity


def test_false_positive_edge_cases():
    """Tests initialization branches, ValueErrors, empty sources, and retry loops."""
