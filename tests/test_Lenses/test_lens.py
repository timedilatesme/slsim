...
class TestLens(object):
    ...
    def test_field_galaxies_integration(self):
        """Test the integration of field galaxies into the lens light models."""
        ...
        fg_model_list, fg_kwargs_list = lens_with_fg.field_galaxy_light_model_lenstronomy(band="i")
        assert len(fg_model_list) > 0, "Expected non-empty model list for field galaxies."
        assert len(fg_kwargs_list) > 0, "Expected non-empty kwargs list for field galaxies."
        ...
        kwargs_model_no_fg, kwargs_params_no_fg = self.gg_lens.lenstronomy_kwargs(band="i")
        
        # The lens with field galaxies should have more light models than the one without
        assert len(kwargs_model_fg["lens_light_model_list"]) > len(kwargs_model_no_fg["lens_light_model_list"])
        assert len(kwargs_params_fg["kwargs_lens_light"]) > len(kwargs_params_no_fg["kwargs_lens_light"])
        
        # Test C: Ensure field_galaxy_light_model_lenstronomy correctly handles None (fallback)
        empty_model_list, empty_kwargs_list = self.gg_lens.field_galaxy_light_model_lenstronomy(band="i")
        assert empty_model_list == [], "Expected empty list when field_galaxies is None."
        assert empty_kwargs_list == [], "Expected empty list when field_galaxies is None."
